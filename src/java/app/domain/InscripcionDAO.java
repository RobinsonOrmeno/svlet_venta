...
public class InscripcionDAO {
    ...
    public List<InscripcionDTO> leerLista() {
     ...
     public boolean modificar(InscripcionDTO ins) {
        PreparedStatement ps = null;
       

        String sql = "UPDATE inscripcion SET id_curso = ?, id_forma_pago = ? WHERE id_inscripcion=?";

        try {
             Connection con = UConnection.getConnection();
            ps = con.prepareStatement(sql);
            
            ps.setInt(1, ins.getIdCurso());
            ps.setInt(2, ins.getIdFormaPago());
            ps.setInt(3, ins.getIdInsc());
            ps.execute();
            return true;
